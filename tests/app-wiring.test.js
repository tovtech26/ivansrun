...
assert.equal(appSource.includes("Ivansrun reseller"), false, "Frontend fallback company copy must use Irunsvan reseller branding.");
assert.equal(indexSource.includes("ivansrun-real-auth-v1"), false, "Index must not keep the stale real-auth cache key after Google OAuth login.");
assert.equal(indexSource.includes("ivansrun-stock-reset-v1"), false, "Index must not keep the stale stock reset cache key after Google OAuth login.");
assert.equal(indexSource.includes("ivansrun-reseller-detail-v3"), false, "Index must not keep the stale reseller detail cache key after stock reset and depletion fixes.");
assert.equal(indexSource.includes("ivansrun-reseller-detail-v2"), false, "Index must not keep the stale reseller detail runtime cache key after layout fix.");
assert.equal(indexSource.includes("ivansrun-reseller-detail-v1"), false, "Index must not keep the stale reseller detail cache key after runtime fix.");
assert.equal(indexSource.includes("ivansrun-bulk-order-v1"), false, "Index must not keep the stale bulk-order cache key after reseller product detail UI fixes.");
...
assert.equal(indexSource.includes("ivansrun-clear-products-v1"), false, "Index must not keep the stale reseller portal cache key.");
...
assert.equal(staticServerSource.includes("ivansrun_auth_session"), false, "Static server must not write auth sessions.");
