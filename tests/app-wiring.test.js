...
assert.equal(appSource.includes("brandAmbassadors:"), true, "App state must track brand ambassadors.");
...
assert.equal(appSource.includes('fetchOptionalSupabase("brand_ambassadors"'), true, "Public bootstrap must fetch published brand ambassadors.");
...
assert.equal(appSource.includes('fetchAuthedSupabase("brand_ambassadors"'), true, "Admin bootstrap must fetch all brand ambassadors.");
...
assert.equal(appSource.includes('"ambassador": brandAmbassadorDetailPage'), true, "Ambassador route must render public profiles.");
assert.equal(appSource.includes('["Products", "product-flyers", ["product-flyers", "product-flyer", "product"]]'), true, "Public navigation must point Products to the public flyer pages, not reseller catalog products.");
assert.equal(appSource.includes('"Footwear"'), false, "Public navigation must not expose the duplicate Footwear label.");
assert.equal(appSource.includes('"Collections"'), false, "Public navigation must not expose the duplicate Collections label.");
assert.equal(appSource.includes("function publicResellerAction("), true, "Public reseller navigation must be role-aware.");
assert.equal(appSource.includes("Become a Reseller"), true, "Signed-out public navigation must use Become a Reseller.");
assert.equal(appSource.includes("My Profile"), true, "Authenticated public navigation must expose My Profile.");
assert.equal(appSource.includes("function navigateToHomeSection("), true, "News and ambassador links must support section navigation.");
assert.equal(appSource.includes("function campaignNewsSection("), true, "Homepage must render a public News section.");
assert.equal(appSource.includes("function campaignAmbassadorsSection("), true, "Homepage must render a public Brand Ambassadors section.");
assert.equal(appSource.includes("function campaignFeaturedFlyers()"), true, "Homepage featured cards must have a dedicated public flyer selector.");
assert.equal(appSource.includes("return publicProductFlyerItems()\n    .filter((flyer) => productFlyerCoverImage(flyer))\n    .slice(0, 3);"), true, "Homepage featured cards must preview the first three public product flyers with cover images.");
assert.equal(appSource.includes('data-route="product-flyer" data-flyer-slug="${escapeHtml(flyer.slug)}" aria-label="View ${escapeHtml(title)}"'), true, "Homepage featured images must open the matching public flyer detail page.");
assert.equal(appSource.includes("New season"), false, "Hero must not show season metadata.");
assert.equal(appSource.includes("Edition 1/1"), false, "Hero must not show edition metadata.");
...
assert.equal(appSource.includes("<h2>Latest News</h2>"), true, "Legacy News renderer should use public-facing News copy.");
...
assert.equal(appSource.includes('data-form="brand-ambassador"'), true, "Admin site controls must include ambassador manager form.");
...
assert.equal(appSource.includes("async function saveBrandAmbassador("), true, "Ambassador saves must have a dedicated handler.");
...
assert.equal(appSource.includes('data-action="delete-brand-ambassador"'), true, "Ambassadors must expose guarded deletion.");
...
assert.equal(appSource.includes("function siteBrandAmbassadorsPanel("), true, "Brand Ambassadors must render in a focused Site Controls panel.");
